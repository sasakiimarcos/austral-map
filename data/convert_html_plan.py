...
def convert_html_plan(html_file, courses_per_sem):
        ...
        if len(df.columns) == 9:
            ...
            while i < size:
                if i + 1 - j >= len(result['Course'].index):
                    ...
                    result = result.drop(i, axis=0)
                    j += 1
                elif re.search('Para cursar', str(result['Course'].iloc[i + 1 - j])):
    ...
    main_plan = pd.concat(courses).set_index('ID')
    main_plan.to_excel('plan.xlsx')
    main_plan.to_json('plan.json', orient="index", indent=2)
